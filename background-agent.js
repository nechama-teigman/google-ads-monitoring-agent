// Google Ads Background Agent - Final Version
...
class GoogleAdsAgent {
  ...
  async createAdDuplicate(customerId, originalAd) {
    ...
    try {
      ...
      // Handle the 3-ad limit error specifically
      if (error.message && error.message.includes('limit on the number of allowed resources')) {
        console.log(`⚠️  Ad group ${adGroupId} has reached the 3-ad limit. Skipping this ad.`);
        return null;
      }
    ...
    try {
      ...
      for (const ad of disapprovedAds) {
        ...
        if (duplicateResult) {
          ...
          console.log(`⚠️  Skipped ad due to 3-ad limit in ad group`);
          skippedCount++;
        }
      ...
      console.log(`\n✅ Monitoring cycle completed. Processed ${processedCount} ads, skipped ${skippedCount} ads across all campaigns`);
